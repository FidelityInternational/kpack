	...
	when("single change processed", func() {
		when("TRIGGER", func() {
			when("date string is given in unsupported format", func() {
				...
				expectedErrorStr := `error validating change for reason 'TRIGGER': parsing time "bad date string" as "2006-01-02 15:04:05.000000 -0700 MST m=+0.000000000": cannot parse "bad date string" as "2006"`
    ...
    "new": [
      {
		...
		when("STACK", func() {
			when("invalid values are given", func() {
				...
				expectedErrorStr := `error validating change for reason 'STACK': could not parse reference: invalid-oldRunImageRef; could not parse reference: invalid-newRunImageRef`
...
[
  ...
  },
  {
    ...
    "new": "sha256:87302783be0a0cab9fde5b68c9954b7e9150ca0d514ba542e9810c3c6f2984ae"
			...
			it("returns the correct ChangeSummary and does not error", func() {
				summary, err := cp.Process(commitChange).Process(triggerChange).Process(stackChange).Process(buildpackChange).Process(configChange).Summarize()
				assert.NoError(t, err)
				assert.True(t, summary.HasChanges)
				assert.Equal(t, v1alpha1.BuildReasonSortIndex, summary.ReasonsStr)
				assert.Equal(t, expectedChangesStr, summary.ChangesStr)
		...
		when("they are all invalid", func() {
			...
			expectedErrorStr := `error validating change for reason 'TRIGGER': parsing time "bad date string" as "2006-01-02 15:04:05.000000 -0700 MST m=+0.000000000": cannot parse "bad date string" as "2006"
error validating change for reason 'STACK': could not parse reference: invalid-oldRunImageRef; could not parse reference: invalid-newRunImageRef`
